...
def train_task1(
    ...
    # MINE: Reduce y coords
    vocabulary = {"action": 0, "other": 1}
    for annotator in dataset:
        arr = dataset[annotator]["annotations"]
        arr[arr < 2.5] = 0
        arr[arr > 2.5] = 1
    for annotator in test_data:
        arr = test_data[annotator]["annotations"]
        arr[arr < 2.5] = 0
        arr[arr > 2.5] = 1
    ...
    input_dim = calculate_input_dim(
        feature_dim, config.architecture, config.past_frames, config.future_frames
    )
    ...
    train_generator = mabe_generator(
        train_data,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--skip-training", action="store_true", help="Only generate metrics"
    )
    ...
    pretrained_model_path = None
    if skip_training:
        model_path = "results\\task1_baseline\\final_task1_seed_42_model.h5"  # f"{results_dir}/task1_seed_{seed}_model.h5"
